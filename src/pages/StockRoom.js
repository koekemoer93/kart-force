...
import { createItem, receiveStock, fulfillRequest } from '../services/inventory';
...
export default function StockRoom() {
  ...
  // forms
  const [newItem, setNewItem] = useState({ name: '', unit: 'pcs', category: 'essentials', minQty: 0, maxQty: 0, initialQty: 0 });
  ...
  async function handleFulfill(id) {
    if (!isAdmin) return alert('Admin only.');
    try {
      await fulfillRequest({ requestId: id, fulfilledBy: user?.uid });
    } catch (err) {
      console.error(err);
      alert(err.message);
    }
      ...
      <div className="main-wrapper admin-dashboard-layout">
              ...
              <div key={it.id} className="track-card">
                <div className="card track">
                  <div className="row between wrap gap12">
                    ...
                    <span className="small muted">{it.category} · {it.unit}</span>
                  </div>
        ...
        <div className="glass-card welcome-card">
          ...
          <form onSubmit={handleCreateItem}>
            <input className="input-field" placeholder="Item name (e.g., Coke 330ml)"
                   value={newItem.name} onChange={(e)=>setNewItem({...newItem, name: e.target.value})}/>
            <div className="row gap12">
              <input className="input-field" placeholder="Unit (e.g., can, pcs)"
            ...
            <div className="row gap12">
              <input className="input-field" type="number" placeholder="Min qty"
                     value={newItem.minQty} onChange={(e)=>setNewItem({...newItem, minQty: e.target.value})}/>
              <input className="input-field" type="number" placeholder="Max qty (optional)"
                     value={newItem.maxQty} onChange={(e)=>setNewItem({...newItem, maxQty: e.target.value})}/>
              <input className="input-field" type="number" placeholder="Initial qty"
                     value={newItem.initialQty} onChange={(e)=>setNewItem({...newItem, initialQty: e.target.value})}/>
            </div>
          ...
          <form onSubmit={handleReceive}>
            <select className="input-field" value={receive.itemId}
                    onChange={(e)=>setReceive({...receive, itemId: e.target.value})}>
              <option value="">Select item…</option>
              {items.map((it) => <option key={it.id} value={it.id}>{it.name}</option>)}
            </select>
            <div className="row gap12">
              <input className="input-field" type="number" placeholder="Quantity"
                     value={receive.qty} onChange={(e)=>setReceive({...receive, qty: e.target.value})}/>
              <input className="input-field" placeholder="Reason (optional)"
                     value={receive.reason} onChange={(e)=>setReceive({...receive, reason: e.target.value})}/>
            </div>
